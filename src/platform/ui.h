...
inline void UI::render(const Editor* editor) const {
    ...
    if (editor && selected_obj >= 0 && selected_obj < obj_count &&
        ...
        drawTextSm(px+12, row, 0.28f, 0.35f, 0.48f, "T:mat  +/-:prop  C/V/B/N:color");
    }

    // Camera info panel (always visible)
    {
        int px = screen_w - PANEL_W - 12;
        int py = HEADER_H + 12;
        if (selected_obj >= 0 && selected_obj < obj_count &&
            selected_obj < (int)editor->scene->objects.size()) {
            py = HEADER_H + 224;
        }
        int panel_h = 52;
        fillRect(px, py, PANEL_W, panel_h, 0.079f, 0.090f, 0.125f, 0.85f);
        strokeRect(px, py, PANEL_W, panel_h, 0.145f, 0.173f, 0.320f, 0.5f);
        fillRect(px, py, 3, panel_h, 0.380f, 0.900f, 0.550f, 0.7f);
        drawTextSm(px+12, py+14, 0.420f, 0.459f, 0.576f, "CAMERA");
        drawTextSm(px+12, py+32, 0.700f, 0.740f, 0.820f, "FOV: %.0f   Exp: %.2f",
            editor && editor->cam ? editor->cam->fov : 55.0f,
            editor && editor->tracer ? editor->tracer->exposure : 2.5f);
        drawTextSm(px+12, py+46, 0.28f, 0.35f, 0.48f, "Z/X: FOV  O/P: Exp");
    }
    ...
    // Export status toast
    if (editor && editor->export_status[0] != 0 && (GetTickCount() - editor->export_status_time) < 3000) {
        int tw = (int)strlen(editor->export_status) * 8 + 24;
        int tx = (screen_w - tw) / 2;
        int ty = screen_h - FOOTER_H - 50;
        fillRect(tx, ty, tw, 28, 0.079f, 0.090f, 0.125f, 0.92f);
        strokeRect(tx, ty, tw, 28, 0.239f, 0.800f, 0.494f, 0.7f);
        drawTextSm(tx+12, ty+18, 0.239f, 0.800f, 0.494f, "%s", editor->export_status);
    } else {
        // Small hint
