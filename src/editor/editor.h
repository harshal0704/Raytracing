...
struct Editor {
    ...
    Editor()
        : scene(nullptr), cam(nullptr), tracer(nullptr), selected(-1), mode(MODE_VIEW),
          ...
          export_status{0}, export_status_time(0) {}
    ...
    bool loadScene(const char* path) {
        ...
        if (ok) { selected=-1; undo_stack.clear(); redo_stack.clear(); scene->updateEmissiveList(); }
        return ok;
    }
    void exportImage(const char* path) {
        if (!tracer) return;
        tracer->exportBMP(path);
        snprintf(export_status, sizeof(export_status), "Exported: %s", path);
        export_status_time = GetTickCount();
    }
    void resetToDefault() {
        pushUndo();
        scene->setupDefault();
        ...
        snprintf(export_status, sizeof(export_status), "Scene reset to default");
        export_status_time = GetTickCount();
    }
    void duplicateSelected() {
        if (selected < 0 || selected >= (int)scene->objects.size()) return;
        pushUndo();
        Object dup = scene->objects[selected];
        dup.center = dup.center + vec3(0.8, 0, 0);
        dup.selected = false;
        scene->objects.push_back(dup);
        selected = (int)scene->objects.size() - 1;
        scene->objects[selected].selected = true;
        scene->updateEmissiveList();
        if (tracer) tracer->needs_reset = true;
        snprintf(export_status, sizeof(export_status), "Object duplicated");
        export_status_time = GetTickCount();
    }
    ...
    void on_key(int vk) {
        if (!scene || !cam) return;
        if (ctrl_down) {
            if (vk=='Z') { undo(); scene->updateEmissiveList(); if(tracer) tracer->needs_reset=true; return; }
            if (vk=='Y') { redo(); scene->updateEmissiveList(); if(tracer) tracer->needs_reset=true; return; }
            if (vk=='S') { saveScene("scene.dat"); return; }
            if (vk=='L') { loadScene("scene.dat"); if(tracer) tracer->needs_reset=true; return; }
            if (vk=='D') { duplicateSelected(); return; }
            if (vk=='E') { exportImage("render.bmp"); return; }
        }
        ...
        switch(vk) {
            ...
            case 'Z':
                cam->fov = clamp(cam->fov + 5.0, 20.0, 120.0);
                cam->update();
                if(tracer) tracer->needs_reset=true;
                snprintf(export_status, sizeof(export_status), "FOV: %.0f", cam->fov);
                export_status_time = GetTickCount();
                break;
            case 'X':
                cam->fov = clamp(cam->fov - 5.0, 20.0, 120.0);
                ...
                break;
            case 'O':
                tracer->exposure = clamp01(tracer->exposure + 0.25f);
                snprintf(export_status, sizeof(export_status), "Exposure: %.2f", tracer->exposure);
                export_status_time = GetTickCount();
                break;
            case 'P':
                tracer->exposure = std::max(0.25f, tracer->exposure - 0.25f);
                snprintf(export_status, sizeof(export_status), "Exposure: %.2f", tracer->exposure);
                export_status_time = GetTickCount();
                break;
        }
        if (selected>=0 && selected<(int)scene->objects.size()) {
            ...
            if (vk==VK_PRIOR) { pushUndo(); obj.center=obj.center+vec3(0,0,-step); }
            if (vk==VK_NEXT)  { pushUndo(); obj.center=obj.center+vec3(0,0,step); }
            if (obj.mat_index>=0 && obj.mat_index<(int)scene->materials.size()) {
                ...
                if (vk=='B') { pushUndo(); mat.albedo=color3(0.15,0.25,0.90); }
                if (vk=='N') { pushUndo(); mat.albedo=color3(0.90,0.65,0.15); }
            }
            if (mode==MODE_SCALE) {
                double s=1.1;
