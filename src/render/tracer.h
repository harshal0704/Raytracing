...
struct Tracer {
    ...
    Tracer()
        : accumulation(nullptr), width(0), height(0),
          ...
          scene_ptr(nullptr), cam_ptr(nullptr), thread_handles(nullptr) {}
    ...
    void init(int w, int h) {
        ...
        if (thread_handles) delete[] thread_handles;
    ...
    void free() {
        if (accumulation) { delete[] accumulation; accumulation=nullptr; }
        if (thread_handles) { delete[] thread_handles; thread_handles=nullptr; }
    }

    // ── Per-mode shading ───────────────────────────────────────────────

    // Full path tracing with Russian Roulette termination and proper direct light sampling
    color3 shade_pathTrace(const ray& r, int depth, XorShift64& rng) {
        ...
        if (scene_ptr->hit(r, 0.001, INF, rec)) {
            ...
            ScatterResult sr;
            if (material_scatter(mat, r, rec, sr, rng)) {
                // Direct light sampling: sample each emissive object
                color3 direct(0,0,0);
                for (int li = 0; li < (int)scene_ptr->emissive_indices.size(); li++) {
                    int obj_idx = scene_ptr->emissive_indices[li];
                    const Object& light_obj = scene_ptr->objects[obj_idx];
                    const Material& lmat = scene_ptr->materials[light_obj.mat_index];
                    vec3 to_light = light_obj.center - rec.p;
                    ...
                    HitRecord sr2;
                    if (!scene_ptr->hit(shadow_ray, 0.001, light_dist * 0.99, sr2)) {
                        double attenuation = lmat.emit_strength * ndotl / (light_dist * light_dist + 1.0);
                        direct = direct + sr.attenuation * attenuation * 0.5;
                    }
                }

                // Russian Roulette after 3 bounces
                color3 indirect(0,0,0);
                if (depth > 3) {
                    double p = std::max(sr.attenuation.x(), std::max(sr.attenuation.y(), sr.attenuation.z()));
                    p = std::max(p, 0.05);
                    if (rng.random01() < p) {
                        indirect = shade_pathTrace(sr.scattered_ray, depth-1, rng) * (sr.attenuation / p);
                    }
                } else {
                    indirect = sr.attenuation * shade_pathTrace(sr.scattered_ray, depth-1, rng);
                }

                // Ambient fill: prevents pure black at low SPP
                color3 amb = 0.04 * scene_ptr->skyColor(unit_vector(rec.normal));

                return direct + indirect + sr.attenuation * amb;
            }
            return color3(0,0,0);
        }
    ...
    // Ambient occlusion
    color3 shade_ao(const ray& r, XorShift64& rng) {
        HitRecord rec;
    ...
    // Dispatch per mode
    color3 shade(const ray& r, int depth, XorShift64& rng) {
        switch(render_mode) {
    ...
    void renderFrame(const Scene& scene, const Camera& cam) {
        ...
        for (int t=0; t<num_threads; t++)
            thread_handles[t] = (HANDLE)_beginthreadex(NULL, 0, renderThreadProc, this, 0, NULL);
        WaitForMultipleObjects(num_threads, thread_handles, TRUE, INFINITE);
        for (int t=0; t<num_threads; t++) CloseHandle(thread_handles[t]);
        samples_per_pixel++;
    }
    ...
    void exportBMP(const char* filename) const {
        ...
        int row_size = width * 3 + pad;
        int file_size = 54 + row_size * height;

        std::vector<unsigned char> bmp(file_size);
        // BMP header
        bmp[0]='B'; bmp[1]='M';
        *(int*)&bmp[2]=file_size;
        *(int*)&bmp[6]=0;
        ...
        *(int*)&bmp[50]=0;
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                color3 col(
                    accumulation[src_idx+0] * scale * exposure,
                    accumulation[src_idx+1] * scale * exposure,
                    accumulation[src_idx+2] * scale * exposure
                );
                if (!is_single_sample_mode) col = tonemap_aces(col);
                double inv_gamma = 1.0/2.2;
                bmp[dst_idx+0] = (unsigned char)(pow(clamp01(col.z()), inv_gamma)*255.99); // B
                bmp[dst_idx+1] = (unsigned char)(pow(clamp01(col.y()), inv_gamma)*255.99); // G
                bmp[dst_idx+2] = (unsigned char)(pow(clamp01(col.x()), inv_gamma)*255.99); // R
            }
        }
        std::ofstream f(filename, std::ios::binary);
        if (f) { f.write((char*)bmp.data(), file_size); f.close(); }
    }
};
