...
struct Scene {
    ...
    void updateEmissiveList() {
        emissive_indices.clear();
        for (int i = 0; i < (int)objects.size(); i++) {
            if (objects[i].mat_index >= 0 && objects[i].mat_index < (int)materials.size()) {
                if (materials[objects[i].mat_index].type == MAT_EMISSIVE &&
                    materials[objects[i].mat_index].emit_strength > 1.0) {
                    emissive_indices.push_back(i);
                }
            }
        }
    }
    ...
    void save(const char* filename) const {
        ...
        for (auto& o : objects) {
            ...
            f.write((char*)&o.normal.e, sizeof(double)*3);
        ...
        for (auto& m : materials) {
            ...
            f.write((char*)&m.albedo2.e,     sizeof(double)*3);
            ...
            f.write((char*)&m.checker_scale, sizeof(double));
        }
    }
    ...
    bool load(const char* filename) {
        ...
        for (int i = 0; i < oc; i++) {
            ...
            f.read((char*)&o.normal.e,  sizeof(double)*3);
        ...
        for (int i = 0; i < mc; i++) {
            ...
            if (version >= 2) f.read((char*)&m.albedo2.e,     sizeof(double)*3);
            ...
            if (version >= 2) f.read((char*)&m.checker_scale, sizeof(double));
    ...
    Scene copy() const {
        Scene s; s.objects=objects; s.materials=materials; s.emissive_indices=emissive_indices;
        s.background_top=background_top; s.background_bot=background_bot; return s;
    }
    void restore(const Scene& s) {
        objects=s.objects; materials=s.materials; emissive_indices=s.emissive_indices;
