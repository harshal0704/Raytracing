...
inline bool hit_box(const Object& obj, const ray& r, double t_min, double t_max, HitRecord& rec) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (t0 > tmin_val) { tmin_val = t0; closest_axis = i; }
    ...
    outward_normal[closest_axis] = (rec.p[closest_axis] - obj.center[closest_axis] > 0) ? 1.0 : -1.0;
    rec.set_face_normal(r, outward_normal);
    rec.mat_index = obj.mat_index;
    return true;
}
...
inline bool hit_cylinder(const Object& obj, const ray& r, double t_min, double t_max, HitRecord& rec) {
    ...
    // End caps
    double cap_y[2] = { obj.center.y() - half_h, obj.center.y() + half_h };
    double cap_norm[2] = { -1.0, 1.0 };
    for (int c = 0; c < 2; c++) {
        if (std::fabs(r.direction().y()) < EPSILON) continue;
        double t = (cap_y[c] - r.origin().y()) / r.direction().y();
        if (t < t_min || t >= closest) continue;
        vec3 p = r.at(t);
        double dx = p.x() - obj.center.x();
        double dz = p.z() - obj.center.z();
        if (dx*dx + dz*dz <= radius*radius) {
            closest = t; hit_any = true;
        }
    }
    ...
    // Compute normal: side or cap?
    double dist_xz = std::sqrt((rec.p.x()-obj.center.x())*(rec.p.x()-obj.center.x()) +
                                (rec.p.z()-obj.center.z())*(rec.p.z()-obj.center.z()));
    vec3 outward_normal;
    if (dist_xz > 0.0 && dist_xz <= radius + EPSILON) {
        // Side surface
        outward_normal = vec3((rec.p.x()-obj.center.x())/radius, 0, (rec.p.z()-obj.center.z())/radius);
    } else {
        // End cap
        outward_normal = vec3(0, (rec.p.y() > obj.center.y()) ? 1.0 : -1.0, 0);
    }
    rec.set_face_normal(r, outward_normal);
    rec.mat_index = obj.mat_index;
    return true;
