...
inline bool material_scatter(const Material& mat, const ray& r_in,
                              const HitRecord& rec, ScatterResult& sr, XorShift64& rng) {
    if (mat.type == MAT_LAMBERTIAN || mat.type == MAT_CHECKER) {
        vec3 scatter_dir = rec.normal + random_unit_vector();
        if (scatter_dir.near_zero()) scatter_dir = rec.normal;
        if (dot(scatter_dir, rec.normal) < 0) scatter_dir = -scatter_dir;
    ...
    if (mat.type == MAT_DIELECTRIC) {
        ...
        vec3 direction;
        if (cannot_refract || schlick(cos_theta, refraction_ratio) > rng.random01())
